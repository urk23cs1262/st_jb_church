...
import { FiUsers, FiBriefcase, FiBookOpen, FiCalendar, FiFileText, FiMessageSquare, FiDollarSign, FiSettings, FiImage, FiBell, FiGift, FiHeart, FiClock } from 'react-icons/fi';
...
export default function AdminDashboard() {
  ...
  const STAT_CARDS = [
    ...
    { 
      ...
      value: stats?.upcomingAnniversaries?.length || 0, 
    ...
    { 
      ...
      subText: `${s.openTickets || 0} tickets & prayers`,
        ...
        {loading ? <SectionLoader /> : (
          <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-church-gold bg-white/10 px-2 py-0.5 rounded-full">
                      TODAY'S SCRIPTURE — {stats.todayVerse.ref}
                    </span>
                    <p className="text-xs sm:text-sm italic text-gray-100 font-serif mt-1">
                      "{stats.todayVerse.verseTextEn || stats.todayVerse.verseTextTa}"
                    </p>
                  </div>
                </div>
              ...
              {STAT_CARDS.map((card, i) => (
                <Link key={i} to={card.link}>
                  <motion.div
                    ...
                    className={`bg-gradient-to-br ${card.color} rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 text-white shadow-lg hover:shadow-2xl hover:scale-[1.01] transition-all flex flex-col justify-between h-full border border-white/10 ${
            ...
            <div className="block md:hidden mb-6">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3 px-1">Quick Access</p>
                ...
                {NAV_ITEMS.map((item, i) => (
                  <Link key={i} to={item.path}>
                    <motion.div
                      ...
                      className="flex flex-col items-center gap-1.5 p-2.5 bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:scale-[1.03] active:scale-95 transition-all"
                    >
                      <div className={`w-9 h-9 rounded-xl ${item.color} flex items-center justify-center text-white text-base shadow-xs`}>
                        {item.icon}
                      </div>
                      <span className="text-[9px] font-bold text-gray-700 text-center leading-tight line-clamp-2">
                        {item.label}
                      </span>
                    </motion.div>
                  </Link>
